...
class ModelRunner:
    ...
    def _format_evaluation(self, username, projectName, logId, checkSummary, checkTerminology, checkHallucination, checkReadability, checkReadabilityScore, checkPurpose, checkPurposeScore, checkProblem, checkProblemScore, checkCreative, checkCreativeScore, checkContradiction, checkContradictionScore, HighLightContradiction, checkStandard, checkPrivacy, HighLightPrivacy, feedback, freqCnt, tokenCnt, letterCnt, byteCnt, eng_list, chi_list):
        ...
        result = {
            ...
            "checkTerminology": checkTerminology,
            "checkHallucination": checkHallucination,
            "checkReadability": checkReadability,
            "checkReadabilityScore": checkReadabilityScore,
            "checkPurpose": checkPurpose,
            "checkPurposeScore": checkPurposeScore,
            "checkProblem": checkProblem,
            "checkProblemScore": checkProblemScore,
            "checkCreative": checkCreative,
            "checkCreativeScore": checkCreativeScore,
            "checkContradiction": checkContradiction,
            "checkContradictionScore": checkContradictionScore,
...
async def receive_data(data: ProcessData):
    ...
    # 평가 저장
    runner = ModelRunner() 
    runner.run_model(username, projectName, logId, result_dict[checkSummary], result_dict[checkTerminology], result_dict[checkHallucination], result_dict[checkReadability], result_dict[checkReadabilityScore], result_dict[checkPurpose], result_dict[checkPurposeScore], result_dict[checkProblem], result_dict[checkProblemScore], result_dict[checkCreative], result_dict[checkCreativeScore], result_dict[checkContradiction], result_dict[checkContradictionScore], result_dict[HighLightContradiction], result_dict[checkStandard], result_dict[checkPrivacy], result_dict[HighLightPrivacy], result_dict[feedback], freqCnt, tokenCnt, letterCnt, byteCnt, eng_list, chi_list)
